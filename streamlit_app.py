...
def detect_columns(master_df: pd.DataFrame, pick_df: pd.DataFrame):
    return {
        ...
        "master_state": find_best_column(master_df, ["state", "province", "region"], banned=[]),
        "master_industry": find_best_column(master_df, ["industry", "c_industry", "companyindustry"], banned=[]),
        "master_departments": find_best_column(master_df, ["department", "departments", "function"], banned=[]),
        "master_position": find_best_column(master_df, ["position", "seniority"], banned=[]),
        "master_companysize": find_best_column(master_df, ["companysize", "company size", "employee"], banned=[]),
        "master_asset_title": find_best_column(master_df, ["asset_title", "asset title", "assetname", "asset"], banned=[]),
        "master_phone": find_best_column(master_df, ["phonemain", "telephone", "phone", "tel", "mobile", "cell"], banned=["phonetic"]),
        "master_website": find_best_column(master_df, ["companyurl", "website", "domain", "web", "url"], banned=["email"]),
        "master_email": find_best_column(master_df, ["email"], banned=[]),
        "master_jobtitle": find_best_column(master_df, ["jobtitle", "job title", "jobtitletext", "title"], banned=[]),

        # PICKLIST
        "pick_country": find_best_column(pick_df, ["country"], banned=["county"]),
        "pick_state": find_best_column(pick_df, ["state"], banned=[]),
        "pick_industry": find_best_column(pick_df, ["companyindustry", "industry"], banned=[]),
        "pick_departments": find_best_column(pick_df, ["function", "department", "departments"], banned=[]),
        "pick_position": find_best_column(pick_df, ["position", "seniority"], banned=[]),
        "pick_companysize": find_best_column(pick_df, ["companysize", "company size"], banned=[]),
        "pick_asset_title": find_best_column(pick_df, ["asset_title", "asset title", "assetname", "asset"], banned=[]),
    }
...
def phone_country_check(raw_phone, country_label) -> tuple[str, str]:
    ...
    for region in regions:
        ...
        try:
            ...
            # NANP tolerance: US/CA acceptable either way
...
def _picklist_values_for_column(pick_df: pd.DataFrame, col_name: str) -> dict:
    """
    Returns: normalized -> exact picklist text
    Filters out empty and obvious instruction rows.
    """
    if col_name not in pick_df.columns:
        return {}

    bad_markers = {
        "donotmap",
        "please map to tal",
        "ref state code mapping doc",
        "use 50% of each",
        "dd/mm/yyyy",
        "text",
    }
    ...
    out = {}
    for raw in pick_df[col_name].tolist():
        s = str(raw).strip()
        if not s:
            continue
        s_norm = norm_picklist_value(s)
        if any(m in s_norm for m in bad_markers):
            continue
        out[s_norm] = s  # bible formatting
    return out


# ------------------ Main processing ------------------
def run_matching(master_bytes: bytes, picklist_bytes: bytes, master_sheet: str, pick_sheet: str, apply_colours: bool, progress_cb=None) -> bytes:
    # logic dataframes (strings only)
    df_master = pd.read_excel(io.BytesIO(master_bytes), sheet_name=master_sheet, dtype=str, keep_default_na=False)
    df_pick = pd.read_excel(io.BytesIO(picklist_bytes), sheet_name=pick_sheet, dtype=str, keep_default_na=False)

    df_out = df_master.copy()
    added_cols = []

    colmap = detect_columns(df_master, df_pick)

    # Picklist "bible" country formatting (only when sure)
    pick_country_col = colmap.get("pick_country")
    picklist_country_label_by_canon = {}
    if pick_country_col:
        for v in df_pick[pick_country_col].tolist():
            v = str(v).strip()
            if v:
                picklist_country_label_by_canon[canon_country_key(v)] = v

    # Standardise only fields that actually have usable picklist lists
    EXACT_PAIRS = [
        ("country", colmap.get("master_country"), colmap.get("pick_country")),
        ("companyindustry", colmap.get("master_industry"), colmap.get("pick_industry")),
        ("function", colmap.get("master_departments"), colmap.get("pick_departments")),
        ("position", colmap.get("master_position"), colmap.get("pick_position")),
        ("companysize", colmap.get("master_companysize"), colmap.get("pick_companysize")),
    ]

    # Try state ONLY if picklist column looks like real values (it doesn’t in your file, so it will naturally skip)
    if colmap.get("master_state") and colmap.get("pick_state"):
        state_map = _picklist_values_for_column(df_pick, colmap["pick_state"])
        if len(state_map) >= 10:  # heuristic: real state lists aren't 3-4 lines of guidance
            EXACT_PAIRS.append(("state", colmap["master_state"], colmap["pick_state"]))

    # originals for change notes
    original_series_by_friendly = {}
    for friendly, master_col, _ in EXACT_PAIRS:
    ...
    for friendly, master_col, pick_col in EXACT_PAIRS:
        ...
        # never touch date/time columns
        if "date" in _clean_header(master_col) or "time" in _clean_header(master_col):
            df_out[out_col] = "Skipped (date/time column)"
            continue

        pick_map = _picklist_values_for_column(df_pick, pick_col)
        if not pick_map:
            df_out[out_col] = "Picklist values not found"
            continue

        matches, new_vals = [], []
        for raw_val in df_master[master_col].tolist():
            v = str(raw_val)
    ...
    # ------------------ Write output: preserve master formatting ------------------
    if progress_cb:
    ...
    # overwrite only standardised master cols (and highlight changes)
    for friendly, master_col, _ in EXACT_PAIRS:
        ...
        for r in range(2, max_row + 1):
            ...
            if i >= len(df_out):
                break
            ws.cell(row=r, column=cidx).value = df_out.at[i, master_col]
            if apply_colours and note_series is not None and str(note_series.iat[i]).strip():
                ws.cell(row=r, column=cidx).fill = CELL_CHANGE

    # write appended columns
    for r in range(2, max_row + 1):
        i = r - 2
        if i >= len(df_out):
            break
        for col_name in ordered_added:
            ws.cell(row=r, column=name_to_newcol[col_name]).value = df_out.at[i, col_name]
        ...
        for r in range(2, max_row + 1):
            ...
            for mc in match_cols:
                if mc in name_to_newcol:
                    set_fill(r, name_to_newcol[mc], norm_text(df_out.at[i, mc]) == "yes")

            if "Company_Domain_Status" in name_to_newcol:
                set_fill(r, name_to_newcol["Company_Domain_Status"],
                         norm_text(df_out.at[i, "Company_Domain_Status"]) in {"likely match", "match"})
            ...
            if "Missing_Required_Fields" in name_to_newcol:
                set_fill(r, name_to_newcol["Missing_Required_Fields"],
                         str(df_out.at[i, "Missing_Required_Fields"]).strip() == "")

            if "Overall_Status" in name_to_newcol:
                set_fill(r, name_to_newcol["Overall_Status"], norm_text(df_out.at[i, "Overall_Status"]) == "pass")

            if "Overall_Issues" in name_to_newcol:
                set_fill(r, name_to_newcol["Overall_Issues"], str(df_out.at[i, "Overall_Issues"]).strip() == "")
...
# ------------------ Streamlit UI ------------------
st.markdown("## Lead Quality Checker (AWS)")
st.caption(
...
if master_file is not None:
    wb = load_workbook(io.BytesIO(master_file.read()), read_only=True)
    master_sheet = st.selectbox("Master sheet to process", wb.sheetnames, index=0)
    master_file.seek(0)

if picklist_file is not None:
    wb = load_workbook(io.BytesIO(picklist_file.read()), read_only=True)
    pick_sheet = st.selectbox("Picklist sheet to use", wb.sheetnames, index=0)
    picklist_file.seek(0)

apply_colours = st.toggle("Colour-code results (green/blue) + yellow headers", value=True)
...
run_btn = st.button(
    ...
    disabled=not (master_file and picklist_file and master_sheet and pick_sheet),
)
...
if st.session_state.last_output_bytes:
    st.download_button(
        ...
        file_name="Full_Check_Results_AWS.xlsx",
